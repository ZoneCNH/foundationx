...
func TestErrorSchemaMatchesFoundationError(t *testing.T) {
	...
	err := foundationx.NewError(
		...
		"contracts.TestErrorSchemaMatchesFoundationError",
		"not available",
	).WithRetryable(true)
	assertJSONKeys(t, err, []string{"kind", "op", "message", "retryable"}, []string{
		"Kind",
		"Op",
		"Message",
		"Cause",
...
func TestHealthSchemaMatchesFoundationHealthStatus(t *testing.T) {
	schema := readSchema(t, "health.schema.json")

	assertStringSet(t, enumFor(t, schema, "status"), []string{
		string(foundationx.HealthHealthy),
		string(foundationx.HealthDegraded),
		string(foundationx.HealthUnhealthy),
	})
	assertStringSet(t, schema.Required, []string{
		"name",
		"status",
		"message",
		"checked_at",
	...
	status := foundationx.NewHealthStatus(
		"contracts",
		foundationx.HealthHealthy,
		"ok",
		time.Unix(0, 0).UTC(),
		7,
	)
	assertJSONKeys(t, status, []string{"name", "status", "message", "checked_at", "latency_ms", "metadata"}, []string{
		"Name",
		"Status",
		"Message",
		"CheckedAt",
...
func TestVersionSchemaMatchesFoundationVersionInfo(t *testing.T) {
	...
	info := foundationx.NewVersionInfo(
	...
	assertJSONKeys(t, info, []string{"module", "version", "commit", "build_time", "go_version"}, []string{
		"Module",
		"Version",
		"Commit",
		"BuildTime",
