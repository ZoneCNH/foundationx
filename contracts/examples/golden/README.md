...
- `obsx-secret-redaction.json`：SecretString 字符串、JSON 与 Sanitize 脱敏行为。
- `lifecycx-rollback-order.json`：启动失败后的反向 Stop 回滚顺序。
- `syncx-first-error.json`：WorkerGroup 首错返回与上下文取消行为。
