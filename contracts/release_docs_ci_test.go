...
func TestReleaseCheckWiresDocumentationAndEvidenceGates(t *testing.T) {
	...
	assertContains(t, makefile, "ci: fmt vet lint test coverage-threshold race bench-check boundary security contracts api-check docs artifact-check dependency-check standard-drift-check workflow-pin-check examples")
	...
	assertContains(t, makefile, "./scripts/ci/workflow-pin-check.sh")
