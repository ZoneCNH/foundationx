...
默认本地验证口径为 `make test`、`make race` 与 `make coverage-threshold`，这些目标刻意排除 `examples` 与 `scripts`，只度量库包。示例入口由 `make examples` 验证；发布就绪必须在干净工作区通过 `make release-final-check` 或 `make release-preflight VERSION=<目标版本>`，并保持 `docs/goal.md` 的必交付工件清单可由本地门禁验证。
